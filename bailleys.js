const { makeWASocket,makeCacheableSignalKeyStore,  DisconnectReason, useMultiFileAuthState, delay } = require('@whiskeysockets/baileys');
    ...
    async connect() { 
        ...
        if (this.pairingcode && !this.sock.authState.creds.registered) {   
            try{
                ...
                this.emit('pairing-code',   {number: this.phone,reason: code});   
            }catch(err){  
                this.emit('close',   {number: this.phone, status:"error",reason: err + ' reconecting | Auto change to qr'});     
